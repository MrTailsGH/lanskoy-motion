    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not os.path.isfile(mp4):
            ...
            if 'not a bot' in err or 'Sign in to confirm' in err:
                # Два разных случая под одной ошибкой, и лечатся по-разному.
                # Через ВПН это почти всегда «узел сгорел»: общий выходной
                # адрес попал в чёрный список, потому что с него ходит много
                # людей. Проверено 21.09.2026 — вечером тот же финский узел
                # качал, утром перестал.
                sys.exit('YouTube требует подтверждения, что вы не бот.\n'
                         'Через ВПН это значит, что выходной адрес сгорел: он '
                         'общий, и с него ходит много народу.\n'
                         'Лечится сменой страны в ВПН-приложении и '
                         'перезапуском того, что качает:\n'
                         '  пульт — закрыть чёрное окно и открыть заново;\n'
                         '  раннер — закрыть run.cmd и запустить снова.\n'
                         'Без ВПН так бывает с датацентрового адреса; с '
                         'домашнего работает.')
